...
Plans:
- [x] 76-01-PLAN.md — Add class + stage gate to BucklerItem.use()
...
Plans:
- [x] 77-01-PLAN.md — Create BoonGate utility, add parry threat propagation, enable conditional sweeping
...
Plans:
- [x] 78-01-PLAN.md — Create SnowballHitMixin with boon-gated slowness and knockback
...
Plans:
- [x] 79-01-PLAN.md — Stack size to 16, copper boat recipes (9 variants)
- [x] 79-02-PLAN.md — Land placement gate for Bastion Stage 5+
- [x] 79-03-PLAN.md — Hostile mob trapping with 4-second breakout
...
- Phase 76: 1 plan (buckler gate) - COMPLETE
- Phase 77: 1 plan (parry aggro + sweeping combined) - COMPLETE
- Phase 78: 1 plan (snowball enhancement) - COMPLETE
- Phase 79: 3 plans (stack/recipes -> land placement -> mob trapping) - COMPLETE
...
- Class + stage gate in item use(): Check ClassManager.getClass() and StageManager.getBoonLevel() before item functionality
- BoonGate utility: Shared class for Stage 3+ checks (DRY pattern)
...
- Hostile mob filter: MobCategory.MONSTER + getTarget() instanceof Player
- Multi-item stack size mixin: extract setStackSize helper method
- MC 1.21 BoatItem API: entityType field (not supplier), AbstractBoat base class
- Boat trapping: UUID-to-tick Map for ephemeral tracking, accessor invoker for protected methods

**Issues Resolved:**

- Vanilla isSweepAttack method private access (replicated logic in mixin)
- MC 1.21 BoatItem API changes (entityType vs supplier)
- Snowball mixin targeting (Snowball.class not Projectile.class)
- Protected method access for AbstractBoat.getDropItem (accessor invoker)
...
_Archived: 2026-02-03 as part of v3.1 milestone completion_
