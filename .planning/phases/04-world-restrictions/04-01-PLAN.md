...
1. checkAdjacency(level: ServerLevel, placementPos: BlockPos): Boolean
   - Returns true if placement is allowed (no nearby allowlist blocks in adjacent positions)
   - Returns false if placement should be blocked (adjacent allowlist block exists)

2. Algorithm - "26 coordinates" means the 26 neighboring blocks (3x3x3 cube minus center):
   - Iterate x,y,z from -1 to +1
   - For each offset position (placementPos + offset):
     - Skip if same position (offset 0,0,0) - this is the center, not a neighbor
     ...
     - If block is in ALLOWED_BLOCKS AND NOT in ADJACENCY_EXEMPT_BLOCKS:
       - Return false (adjacent to another restricted allowlist block)
   - If no conflicts found in the 26 neighbors, return true

3. This is exactly 26 block checks (3*3*3 - 1 = 26), very efficient.
   The "26 coordinates" in the requirement refers to the 26 positions adjacent to a block
   (face neighbors + edge neighbors + corner neighbors), NOT a distance of 26.
...
- [ ] checkAdjacency scans 26 neighboring blocks (3x3x3 cube minus center)
