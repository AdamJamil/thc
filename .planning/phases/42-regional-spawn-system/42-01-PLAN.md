...
must_haves:
  truths:
    - "Surface spawns have 5% chance of witch instead of vanilla mob"
    - "Upper cave spawns have custom distribution (witch, vex, pillagers) mixed with vanilla"
    - "Lower cave spawns have custom distribution (blaze, breeze, vindicator, pillager, evoker) mixed with vanilla"
    - "Custom spawns use pack size 1-4"
    - "Vanilla fallback spawns proceed unchanged"
    - "Melee pillagers spawn with iron sword, ranged with crossbow"
  artifacts:
...
3. Create static Map<String, List<WeightedEntry>> TABLES with distributions:
   ...
   - END: 25 endermite, 75 vanilla fallback
...
1. Add region detection method (reuse logic from MobFinalizeSpawnMixin):
   - detectRegion(ServerLevel level, BlockPos pos) -> String
   - Y < 0 = OW_LOWER_CAVE
   - Y >= heightmap = OW_SURFACE
   - Otherwise = OW_UPPER_CAVE
   - Check dimension: only Overworld and End have custom distributions

2. In thc$replaceWithSurfaceVariant redirect, add regional distribution check FIRST:
   - Get region from entity position
   - Call SpawnDistributions.selectMob(region, level.random)
   - If vanilla fallback: proceed with existing husk/stray replacement logic
...
4. For pack spawning with custom mobs:
   - When custom mob selected, roll pack size: 1 + random.nextInt(4) for [1,4] uniform
   - First mob uses original position
   - Additional pack members offset by random (-5 to +5) on X/Z, adjust Y to heightmap
   - Create each mob, apply variant equipment if pillager
   - Add all to level via addFreshEntityWithPassengers
...
grep confirms SpawnDistributions.selectMob call in redirect method
  ...
  <done>
SpawnReplacementMixin intercepts spawns, rolls regional distribution, creates custom mobs with pack spawning [1,4], applies pillager equipment, falls back to existing husk/stray logic for vanilla fallback.
  </done>
</task>

<task type="auto">
  <name>Task 3: Add End dimension support and verify integration</name>
  <files>src/main/java/thc/mixin/SpawnReplacementMixin.java</files>
  <action>
1. Update dimension checking:
   - Overworld: use regional distribution (OW_SURFACE, OW_UPPER_CAVE, OW_LOWER_CAVE)
   - End: use "END" distribution (25% endermite)
   - Nether: skip distribution, vanilla only
...
3. Verify integration points:
   ...
   - Regional distribution runs on all spawns, rolls custom vs vanilla
...
4. Add documentation comments explaining the priority order:
  ...
  <verify>
Build passes: `./gradlew build`
grep for "END" or "Level.END" in SpawnReplacementMixin
Documentation comments present explaining spawn priority order
  </verify>
  <done>
End dimension spawns 25% endermite, Nether spawns vanilla, Overworld uses regional distributions. NBT tagging via MobFinalizeSpawnMixin works for all custom spawns. Priority order documented.
  </done>
</task>
...
5. Pack spawning logic present for custom mobs
6. End dimension handling present
</verification>
...
5. Custom mobs spawn in packs of 1-4
6. Vanilla fallback proceeds to existing husk/stray logic
7. End dimension uses endermite distribution
</success_criteria>
