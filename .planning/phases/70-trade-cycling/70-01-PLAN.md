...
**Step 1: Validation (no emerald consumed yet)**
In the 0 XP path (where it currently returns PASS), add validation:
...
**Step 2: Handle poolSize <= 1 (failure path — do NOT consume emerald)**
If poolSize <= 1:
- Play failure effects (VILLAGER_NO sound)
- Return SUCCESS (blocks GUI opening)
- CRITICAL: Do NOT call stack.shrink(1) — no emerald consumed

**Step 3: Handle poolSize > 1 (success path — consume emerald)**
If poolSize > 1:
- Consume emerald: stack.shrink(1)  ← emerald consumed HERE, after validation passes
...
**Step 4: Add private function `cycleCurrentLevelTrades(villager, profKey, currentLevel, serverLevel)`**
...
**Step 5: Add helper functions for feedback effects**
