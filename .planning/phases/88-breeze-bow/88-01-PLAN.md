...
must_haves:
  truths:
    ...
    - "Breeze Bow arrows have drag factor 0.01, traveling farther horizontally than wooden/blaze arrows (0.015)"
  ...
  key_links:
    ...
    - from: "BowType.kt"
      ...
      via: "fromBowItem returns BREEZE with dragFactor=0.01, applied per-tick by ProjectileEntityMixin"
      pattern: "BREEZE.*0\\.01"
...
<objective>
Add the Breeze Bow as a Support-class weapon with 75% damage, regular knockback, fast draw speed (0.75x), drag factor 0.01 (longer range), and Support Stage 2+ class gating.

Purpose: Complete the third bow tier for v3.4, giving Support players a fast-draw utility bow with knockback for crowd control and extended horizontal range.
Output: Working Breeze Bow item with textures, recipe, damage profile, knockback, drag factor, draw speed, and class gating.
</objective>
...
<task type="auto">
  <name>Task 1: Breeze Bow item, textures, recipe, registration, and drag factor wiring</name>
  <files>
    src/main/kotlin/thc/bow/BreezeBowItem.kt
    src/main/kotlin/thc/bow/THCBows.kt
    src/main/kotlin/thc/bow/BowType.kt
  ...
  3. **Update `BowType.kt` -- CRITICAL for drag factor (PHYS-02)**:
     - Phase 86 created `BowType.BREEZE(dragFactor=0.01)` in the enum. The drag factor of 0.01 (vs 0.015 for wooden/blaze) is what gives the Breeze Bow longer horizontal arrow range
     - Update `fromBowItem()` to return `BowType.BREEZE` when the stack is a Breeze Bow item (i.e., `stack.item is BreezeBowItem` or equivalent check matching the Blaze Bow pattern)
     - This is what activates the drag factor: when `ProjectileEntityMixin` calls `BowType.fromBowItem()` during `shoot`, it gets `BREEZE` and stores `dragFactor=0.01`. The per-tick drag in `ProjectileEntityMixin` then uses this value, resulting in: `drag_coefficient = max(0.8, 1 - 0.01 * ticks_in_flight)`. With 0.01, the coefficient reaches 0.8 floor at tick 20 (vs tick 14 for wooden/blaze at 0.015), meaning Breeze Bow arrows maintain higher horizontal speed for longer
     - If Phase 86/87 added item references to the enum, ensure BREEZE has a reference to the BreezeBowItem
  ...
  <verify>
  `./gradlew build` compiles without errors. Verify: breeze_bow.json item definition exists, 4 model files exist, 4 texture files exist, recipe file exists, en_us.json contains "Breeze Bow", BreezeBowItem.kt has class gating check for Support + Stage 2. Verify BowType.kt `fromBowItem()` returns BREEZE for BreezeBowItem (grep for the mapping).
  </verify>
  <done>
  Breeze Bow item registered and craftable (ITEM-05, ITEM-06). Custom textures with pulling states render in inventory. Recipe: 3 breeze rods + 3 string. Non-Support or sub-Stage-2 players see "The bow gusts are beyond your control." and cannot draw (GATE-02). Draw speed is 0.75x wooden bow (MECH-02). BowType.fromBowItem() returns BREEZE for Breeze Bow, activating drag factor 0.01 in ProjectileEntityMixin (PHYS-02).
  </done>
</task>
...
- Breeze Bow arrows travel farther horizontally than Wooden/Blaze bow arrows (drag factor 0.01 vs 0.015)
- Tipped arrows fire normally from Breeze Bow (no restriction, no splash AoE)
</verification>

<success_criteria>
Breeze Bow is a fully functional Support-class weapon: craftable, class-gated, fast-drawing, with 75% damage, preserved knockback, and lower drag factor (0.01) for extended range. All 7 requirements (ITEM-05, ITEM-06, DMG-02, DMG-04, MECH-02, GATE-02, PHYS-02) are satisfied. No splash AoE behavior present (deferred).
</success_criteria>
