...
public final class LootTableEvents {
	...
	public static final Event<Replace> REPLACE = EventFactory.createArrayBacked(Replace.class, listeners -> (key, original, source, registries) -> {
		for (Replace listener : listeners) {
			@Nullable class_52 replaced = listener.replaceLootTable(key, original, source, registries);
	...
	public interface ModifyDrops {
		 ...
		 * @param entry the loot table's registry entry. This will be a {@link class_6880.class_6883} if the lootTable is registered, or a {@link class_6880.class_6881} if the table is inline
		 * @param context the loot context for the current drops
		 * @param drops the list of drops from the loot table to modify
		 */
		void modifyLootTableDrops(class_6880<class_52> entry, class_47 context, List<class_1799> drops);
	}
}
