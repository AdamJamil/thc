...
object JobBlockAssignment {
    ...
    private fun assignNearestVillagerToJobSite(
        ...
        // 2. Set JOB_SITE memory (bypass claim check for explicit assignment)
        ...
        // Trades are auto-generated by vanilla when setVillagerData changes level
        // Do NOT call updateTrades() manually - it causes duplicates
        val offersAfter = villager.offers.size
        msg("§7Trades generated: $offersAfter")
        ...
        // Refresh brain to recognize new job site and profession
        villager.refreshBrain(level)

        // Check state AFTER refreshBrain
        val postRefreshProf = villager.villagerData.profession.unwrapKey().orElse(null)
        val postRefreshJobSite = villager.brain.getMemory(MemoryModuleType.JOB_SITE)
        msg("§7After refreshBrain: prof=$postRefreshProf jobSite=${postRefreshJobSite.isPresent}")
